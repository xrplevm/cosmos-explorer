...
interface RemoveValidatorValue {
  authority?: string;
  sender?: string;
...
export function RemoveValidatorOverviewCard({
  hash,
  transaction,
  chainConfig,
  ...
  const parsed = parseJsonIfString(firstMessage?.value) as
    | RemoveValidatorValue
    | null
    | undefined;
  const root = parsed ?? {};
  const data = root.data ?? {};

  const authority = root.authority ?? root.sender ?? data.authority ?? data.sender;
  const validator =
