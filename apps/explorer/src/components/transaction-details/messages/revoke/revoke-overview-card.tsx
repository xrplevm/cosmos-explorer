...
export function RevokeOverviewCard({
  hash,
  transaction,
  chainConfig,
  ...
  const msgTypeUrl =
    root.msg_type_url ?? root.msgTypeUrl ?? data.msg_type_url ?? data.msgTypeUrl;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-0">
        ...
        {msgTypeUrl != null && msgTypeUrl.length > 0 ? (
          <>
            <Separator />
            <DetailRow label="Message Type">
              <span className="font-mono text-xs break-all">{msgTypeUrl}</span>
            </DetailRow>
          </>
        ) : null}
