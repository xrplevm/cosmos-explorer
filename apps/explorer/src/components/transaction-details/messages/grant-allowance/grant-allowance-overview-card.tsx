...
interface GrantAllowanceValue {
  granter?: string;
  grantee?: string;
  allowance?: { "@type"?: string; type?: string };
  data?: {
    granter?: string;
    grantee?: string;
    allowance?: { "@type"?: string; type?: string };
...
export function GrantAllowanceOverviewCard({
  hash,
  transaction,
  chainConfig,
  ...
  const allowance = root.allowance ?? data.allowance;
  const allowanceTypeUrl = allowance?.["@type"] ?? allowance?.type;
  const allowanceType =
    typeof allowanceTypeUrl === "string" && allowanceTypeUrl.length > 0
      ? shortType(allowanceTypeUrl)
      : undefined;
    ...
    <Card>
      ...
      <CardContent className="space-y-0">
        ...
        {allowanceType != null ? (
          <>
            <Separator />
            <DetailRow label="Allowance Type">
